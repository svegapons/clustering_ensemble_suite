...
{
    public abstract class GenericDistances
    {
    ...
    public class MirkinDistance : GenericDistances
    {
    ...
    public class LatticeDistance : GenericDistances
    {
    ...
    //First Heuristics
    public class BOK : Mirkin
    {
        public BOK(Set set, List<Structuring> estr)
            : base(set, estr)
        { Name = "Best of k (BOK)"; }
        public BOK() : base() { Name = "Best of k (BOK)"; }
    ...
    public class SAOM : Mirkin
    {
        ...
        public SAOM(Set set, List<Structuring> estr)
            : base(set, estr)
        { Name = "Simulated Annealing One-element Move (SAOM)"; }
        public SAOM() : base() { Name = "Simulated Annealing One-element Move (SAOM)"; }
        ...
        public override Structuring BuildStructuring()
        {
            try
            {
                ...
                string[] _result_labels = SA<string[]>.RunSAOM(Set, initial_sol, labels_List, Temperature, .99, 50, 1.05, 10, .5, IterationsCount, IContainerProgressBar, _current, GenericDistances, ref _maxCluster);
    ...
    public class BOM : Mirkin
    {
        ...
        public BOM(Set set, List<Structuring> estr)
            : base(set, estr)
        { Name = "Best One-element Move (BOM)"; }
        public BOM() : base() { Name = "Best One-element Move (BOM)"; }
        ...
        public override Structuring BuildStructuring()
        {
            try
            {
                ...
                {
                    IContainerProgressBar.ResetProgressBar(1, _max, true);
                    IContainerProgressBar.UpdateProgressBar(0, "Running BOM algorithm...", true);
                }
                ...
                string[] _result_labels = SA<string[]>.RunBOM(Set, initial_sol, labels_List, Temperature, .99, 50, 1.05, 10, .5, IterationsCount, IContainerProgressBar, _current, GenericDistances, ref _maxCluster);
                Dictionary<string, Cluster> dic_clusters = new Dictionary<string, Cluster>();
                for (int i = 0; i < Set.ElementsCount; i++)
                {
                    if (dic_clusters.ContainsKey(_result_labels[i]))
            ...
            {
                if (IContainerProgressBar != null)
                    IContainerProgressBar.ShowError("Error occurred in BOM algorithm.");
                return null;
            }
        }
    }
    ...
    //Third Heuristics
    public class SAOMNewNeighbor : Mirkin
    {
        [In(typeof(IntGTZeroConverter))]
        public int IterationsCount { get; set; }
        ...
        public SAOMNewNeighbor(Set set, List<Structuring> estr)
            : base(set, estr)
        { Name = "Simulated Annealing MeetJoinClusters"; }
        public SAOMNewNeighbor() : base() { Name = "Simulated Annealing MeetJoinClusters"; }
    ...
    public delegate T Neighbor<T>(T t, object[] parameters);
    public delegate double EvaluateSolution<T>(T t, object[] parameters);
    public class SA<T>
    {
        /// <summary>
    ...
    public class DelegateImplementations
    {
        ...
        public static string[] MeetJoinClusters(Set Set, string[] labels, ref int aMaxCluster)
        {
            Random _rnd = new Random(Environment.TickCount);
            Random _rnd1 = new Random(Environment.TickCount);
            Random _rnd2 = new Random(Environment.TickCount);
            double prob = 0;
            ...
            if ((prob = _rnd.NextDouble()) < 0.5)
            {
                if (aMaxCluster > 1)
                {
                    int pos1 = _rnd1.Next(0, labels.Length);
                    string label1 = labels[pos1];
                    string label2 = null;

                    while ((label2 = labels[_rnd2.Next(0, labels.Length)]) == label1)
                    { }
                    for (int i = 0; i < _result.Length; i++)
                        _result[i] = (_result[i] == label1 || _result[i] == label2) ? label1 : _result[i];
            ...
            {
                if (aMaxCluster < labels.Length)
                {
                    int pos1 = _rnd1.Next(0, labels.Length);
                    string label1 = labels[pos1];                    
                    string newlabel = label1 + "-" + aMaxCluster;

                    bool newclust = false, oldlabel = false;
                    for (int i = 0; i < _result.Length; i++)
                    {
                        ...
                        if (_result[i] == label1)
                        {
                            ...
                            double divProb = _rnd2.NextDouble();
                            if (divProb < .5)
                            {
                                oldlabel = true;
                                _result[i] = label1;
